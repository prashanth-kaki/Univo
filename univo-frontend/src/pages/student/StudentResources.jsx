...
const StudentResources = () => {
  ...
  const subjects = ['All', ...new Set(resources.map(r => r.subjectName || r.subject))].filter(Boolean).sort();
  const filteredResources = selectedSubject === 'All' 
    ? resources 
    : resources.filter(r => (r.subjectName || r.subject) === selectedSubject);
    ...
    <div className="max-w-7xl mx-auto space-y-6 h-[calc(100vh-8rem)] flex flex-col">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Access your subject materials and resources.</p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
        <div className="w-full lg:w-64 shrink-0 bg-white rounded-xl border border-slate-200 p-4 h-fit max-h-full overflow-y-auto">
              ...
              <li key={sub}>
                <button 
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-between ${
                    selectedSubject === sub ? 'bg-violet-50 text-violet-700' : 'text-slate-600 hover:bg-slate-50'
                  }`}
                >
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
             ...
             <div className="text-center py-20 text-slate-500 bg-white rounded-xl border border-slate-200 h-full flex items-center justify-center">
               No resources found for this subject.
             </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 pb-4">
              {filteredResources.map(res => <ResourceCard key={res._id || res.id} resource={res} />)}
            </div>
          )}
        </div>
      </div>
