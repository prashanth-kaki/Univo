...
const AdminSidebar = ({ isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    ...
    { path: '/admin/moderation', icon: <ShieldAlert size={20} />, label: 'Moderation' },
    { path: '/admin/reports', icon: <Flag size={20} />, label: 'Reports' },
    { path: '/admin/analytics', icon: <BarChart3 size={20} />, label: 'Analytics' },
    { path: '/admin/activity', icon: <Activity size={20} />, label: 'Activity Logs' },
  ...
  return (
    <motion.div 
      ...
      className="fixed left-0 top-0 h-full bg-slate-900 text-slate-300 z-50 flex flex-col shadow-xl overflow-hidden transition-all duration-300"
    >
      <div className={cn("p-6 flex items-center border-b border-slate-800 h-16", isOpen ? "justify-start" : "justify-center")}>
        <div className="flex items-center gap-3 text-indigo-400">
          <ShieldAlert size={28} className="shrink-0" />
          {isOpen && <span className="text-xl font-bold tracking-tight text-white whitespace-nowrap">Super Admin</span>}
        </div>
      </div>
      ...
      <nav className="flex-1 py-6 overflow-y-auto overflow-x-hidden flex flex-col gap-1 px-3 custom-scrollbar">
        {menuItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className={cn("shrink-0 transition-colors", isActive ? "text-indigo-400" : "group-hover:text-white")}>
                  {item.icon}
                </span>
                {isOpen && (
