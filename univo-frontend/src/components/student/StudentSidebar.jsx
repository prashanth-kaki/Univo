...
import { studentSidebarData } from '../../data/studentSidebar';
    ...
    >
      <div className={cn("p-6 flex items-center border-b border-slate-800 h-16 relative", isOpen ? "justify-start" : "justify-center")}>
        ...
        <button 
          ...
          className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-violet-600 rounded-full flex items-center justify-center text-white hover:bg-violet-500 transition-colors shadow-md z-50 opacity-0 group-hover:opacity-100 hidden md:flex"
        >
        ...
        {studentSidebarData.map((item) => (
          ...
          >
            {({ isActive }) => {
              ...
              return (
                <>
                  <span className={cn("shrink-0 transition-colors", isActive ? "text-white" : "text-slate-400 group-hover:text-violet-400")}>
