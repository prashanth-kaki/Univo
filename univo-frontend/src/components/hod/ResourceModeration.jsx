...
const ResourceModeration = ({ queue, loading }) => {
  if (loading) {
        ...
        {queue?.map(item => (
          <div key={item.id} className="p-4 rounded-xl border border-rose-100 bg-white shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center gap-2 border-t border-slate-100 pt-3">
              <button className="flex-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 py-1.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-colors">
                <Check className="w-4 h-4" /> Approve
              </button>
              <button className="flex-1 bg-rose-50 text-rose-700 hover:bg-rose-100 py-1.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-colors">
